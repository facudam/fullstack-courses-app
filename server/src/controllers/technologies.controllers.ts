...
const getTechnologyById = async(req: Request, res: Response) => {
    try {
        const [ result ]  = await pool.query<ResultSetHeader[]>('SELECT * FROM technologies WHERE tech_id = ?', [req.params.id])
...
const createTechnology = async(req: Request, res: Response) => {
    try {
        ...
        pool.query('INSERT INTO technologies (tech_name) VALUES (?)', [tech_name]);
...
const updateTechnology = async(req: Request, res: Response) => {
    try {
        ...
        const [result] = await pool.query<ResultSetHeader>('UPDATE technologies SET tech_name = IFNULL(?, tech_name) WHERE tech_id = ?', [tech_name, id])
...
const deleteTechnology = async(req: Request, res: Response) => {
    try {
        const { id } = req.params;
        const [ result ] = await pool.query<ResultSetHeader>('DELETE FROM technologies WHERE tech_id = ?', [ id ]);
        if (result.affectedRows <= 0) return res.status(404).json({ message: 'Technology not found' })
